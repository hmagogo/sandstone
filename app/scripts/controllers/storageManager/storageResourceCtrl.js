...
(function () {
    'use strict';
    angular.module('sdsomwebApp').controller('storageResourceCtrl', storageResourceCtrl);

    storageResourceCtrl.$inject = ['$rootScope', '$scope', 'storageFactory', 'mainFactory', 'configFactory'];

    function storageResourceCtrl($rootScope, $scope, storageFactory, mainFactory, configFactory) {
        ...
        $scope.selectPoolCreateVolume = function (pool) {
            $scope.storage.CreateVolumeCurrentPool = {};
            if (pool){
                for (var i=0; i<$scope.storage.poolList.length;i++){
                    if ($scope.storage.poolList[i]['poolName'] == pool.poolName) {
                        $scope.storage.CreateVolumeCurrentPool = $scope.storage.poolList[i];
                        return
                    }
                }
            }
        ...
        function showMapSnap(event, currentSnap){
            ...
            for (var i=0; i< $scope.storage.lunList.length; i++){
                if(name == $scope.storage.lunList[i].volumeName){
        ...
        function deleteTarget(){
            if($scope.storage.isYesConfirm == 'YES'){
        ...
        /** 页面初始化 **/
        function init() {
            $scope.gMainStateRouteView.current = 'main.storage_view';
            _this.viewCtrl                = 'poolView';
